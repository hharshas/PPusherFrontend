  ...
  const Home = () => {
    ...
    const mergeAndPlaySongs = async () => {
      try {
        ...
        const audioBlob = await exportAudioBufferAsWav(mergedBuffer, audioContext);
    ...
    const exportAudioBufferAsWav = async (audioBuffer, audioContext) => {
      ...
      const wavBlob = new Blob([renderedBufferToWav(renderedBuffer)], { type: 'audio/wav' });
      return wavBlob;
    };
    ...
    return (
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        <ul className="max-h-full overflow-y-auto">
          {searchResults.map((song, index) => (
            <li key={index} className="mb-4 border border-gray-300 rounded p-4 flex justify-between items-center">
              <span><strong>Song Name: </strong> {song.fileName}</span>
      ...
      <button
        ...
        disabled={isPlaying}
        className="fixed bottom-0 left-0 w-full bg-blue-500 text-white p-4 hover:bg-blue-600 focus:outline-none"
      >
        {isPlaying ? (
